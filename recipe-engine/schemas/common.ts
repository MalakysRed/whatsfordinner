...
/**
 * `archetype_step.id` is the one authored ID with no prefix assigned in either
 * SPEC.md §4 or the `CLAUDE.md` prefix list, so it is held to the general
 * uppercase shape only. Give it a prefix in SPEC.md and this tightens to
 * `prefixedId(...)` like the rest.
 */
export const authoredIdSchema = z
  .string()
  .regex(/^[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)*$/, "must be an uppercase identifier");
